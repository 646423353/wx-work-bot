...
- **消息接收服务**: 接收企业微信推送的消息，支持企业微信加密消息解密
- **消息处理引擎**: 解析和存储消息数据到 SQLite 数据库
- **告警判断引擎**: 检测未回复消息并触发告警
- **Web管理界面**: Vue3 + Element Plus 构建的管理后台
- **数据存储**: SQLite 轻量级数据库，数据持久化存储
- **用户认证**: JWT Token 认证机制，支持多用户登录
...
系统使用 **SQLite** 数据库，数据存储在 `data/wxwork.db` 文件中。
